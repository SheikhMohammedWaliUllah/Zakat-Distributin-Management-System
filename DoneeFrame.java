...
public class DoneeFrame extends JFrame{
	private Container c;
	...
	public void itsComponent() {
		...
		headline = new JLabel("Donee Information");
		...
		submitButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e)  {
				...
				try {
					...
					while(scan.hasNext()) {
						String NIDTemp = scan.next();
						if(NIDno.equals(NIDTemp)) {
							m = 1;
						}
					}

				}
				catch(Exception ex) {   // polymorphism

				}

				if(m==0) {
					if(mainBalance < RulesAndRegulations.balanceLiminit) {
						try {


							Donee.DoneeInfoStore(new Donee(name, age, NIDno, phoneNo, accNo, bankName, mainBalance));
						...
						JOptionPane.showMessageDialog(null, "You're not eligible as a donee");
					}

				}
				else {
					JOptionPane.showMessageDialog(null,"You have already registered -admin");
				}
			}
